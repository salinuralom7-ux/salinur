...
approve service expert profiles, take every live service expert down, or read the WhatsApp
verification codes.
...
Right now a service expert only finds out about a booking by opening MySheher and
...
issued against it, so every service expert who had tapped *Turn on alerts* would
...
service expert for permission, so nobody is ever subscribed and the queue stays
...
1. On your phone, open MySheher, sign in as a service expert, go to your profile.
...
3. **Close MySheher completely** — swipe it away, do not just go to the home
   screen.
4. From another phone, book that service expert.
...
whether a key is set, how many service experts and customers are subscribed, how many
...
- **No row at all** — the service expert never subscribed. Go back to step 2 above.
...
A service expert is never notified about their own messages, and a second message in
...
| | |
|---|---|
| The service expert accepts | "Bhaskar accepted your booking — Carpenter is confirmed. Arriving in about 30 minutes." |
| The service expert declines | "Bhaskar could not take this one" plus their reason |
| The service expert starts | "Bhaskar has started — Carpenter is under way." |
| The job is finished | "Work finished — How did Bhaskar do?" |
| The service expert replies | "Bhaskar replied — On my way at 10." |
...
3. Tap **Service experts & Pages**
4. Click **Create**

> Same as before: **Service experts & Pages** is inside **Compute**, not on the top
> level.
...
2. Tap **Register as a service expert** → **Create account**
3. Register a test service expert: any name, your own WhatsApp number, a 4-digit PIN
...
test service expert's photo shows up on their profile. It will have gone to Supabase,
...
8. Delete the test service expert: open the profile, scroll down, **Delete my profile**
...
The QR code on every printed service expert ID card issued before today points at
