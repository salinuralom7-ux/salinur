...
| Field | Answer |
|---|---|
| App name | `MySheher: Local Workers Nearby` |
...
4. Release notes: *First release. Find and book local workers in Guwahati.*
...
Friends, family, the first workers who register. They must each **accept the
...
* **App access** — everything is reachable without signing in except a
  worker's own dashboard. Say so, and give a test worker's number and PIN so
...
2. **The reviewer could not get in.** Give them a working worker number and
